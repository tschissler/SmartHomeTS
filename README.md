...
```Please be aware that not all of these projects are used / maintained. Some of them have been replaced by other options and are just kept for documentation purposes.```
...
- **Purpose**: This project allows to control room temperatures by viewing / changing thermostat settings. The display integrates fully via MQTT as it receives all data to be displayed by subscribing to various MQTT topics and sending changes to the thermostat settings as MQTT messages.

...
| 12 Energieaufteilung | `laden-12-energieaufteilung` | 2026-09-20 | **erledigt, gemergt und ausgerollt** (`44c0e80`, ChargingController `1.0.50`). **Mechanik verifiziert, Rechnung NOCH NICHT:** Der Controller publiziert `daten/Laden/M3/<Box>/Energieaufteilung` im Takt, der Payload ist vollständig, die Zähler stehen sauber bei 0,0000 statt bei Müll. Seit dem Rollout um 11:41 UTC hat aber kein Fahrzeug geladen — der VW war voll, die Garage leer. **Offene Nachkontrolle: Summe der drei Quellen gegen die gemessene `Ladeleistung`, bei der nächsten echten Ladung.** Solange gilt Punkt 12 nicht als geprüft | Tests **94/94 grün** (48 neu), von mir selbst nachgelaufen; kein Erwartungswert der 46 Alttests angefasst. Neues Topic `daten/Laden/M3/<Box>/Energieaufteilung`, retained. Löst **sechs** Rollouts aus (`SharedContracts`) |
